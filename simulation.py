...
class Simulation(animation.TimedAnimation):
    ...
    def __init__(self, n_particles, mass, rad, T, V, max_time):
        """
    ...
    def evaluate_properties(self):
        ...
        self.U = (3/2) * self.PART * k_B * self.T
    ...
    def init_particles(self):
        ...
        """
        self.r = np.random.rand(self.PART, 3) * 2 * (self.L/2 - self.RAD) - (self.L/2 - self.RAD)
    ...
    def _draw_frame(self, t):
        ...
        if t > 1 and int(t / self.dt) % self.dP == 0:
            ...
            self.line_p.set_data(self.P_x[:self.last_P + 1], self.P_y[:self.last_P + 1])
            self.ax6.set_ylim(np.min(self.P_y), np.max(self.P_y))
